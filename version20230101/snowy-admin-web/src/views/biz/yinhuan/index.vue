...
<!--								增加隐患	-->
									...
									<a-button danger @click="exportBatchYinhuan()" v-if="hasPerm('delete_danger')">{{
											$t('common.exportButton')
										}}</a-button>
...
<script setup name="yinhuanAPI">
	...
	import exportApi from '@/api/dev/exportApi'
	...
	const exportBatchYinhuan = () => {
		...
		exportApi.exportExcel().then((res) => {

			const fileName = res.headers['Content-disposition'];
			const url = window.URL.createObjectURL(new Blob([res.data]));
			const link = document.createElement('a');//下载文件通过a标签下载
			link.href = url;
			link.setAttribute('download', "治理台账表.xls");
			document.body.appendChild(link);
			link.click();
				/*let path = res.split('=')[1]
				console.log(res)*/
				/*;(formData.value.troubles_image = path), */
			// console.log(res);
