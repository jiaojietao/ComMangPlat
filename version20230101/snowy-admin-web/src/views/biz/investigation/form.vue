					...
					<a-row :gutter="16">
						...
						<a-col :span="8">
							<a-form-item label="完成时间：" name="complete_time">
								<a-date-picker v-model:value="formData.complete_time" placeholder="完成时间" value-format="YYYY-MM-DD" allow-clear />
							</a-form-item>
						</a-col>
					</a-row>
					<a-row :gutter="16">
...
<script setup>
	...
	const customRequestLocal_update = (data) => {
		...
		const fileData = new FormData()
		fileData.append('file', data.file)
		fileApi.fileUploadYinhuanReturnUrl(fileData)
			.then((res) => {
				let path=res.split("=")[1]
				// console.log(path)
				formData.value.update_image= path,
