...
//          return null;
    ...
    public void add(safetyAddParam safetyaddParam) {
        ...
        BizUser ORG = bizUserService.queryEntity(StpUtil.getLoginIdAsString());//查询的是用户信息

        safety safety = BeanUtil.toBean(safetyaddParam, safety.class);//参数信息

        safety.setOrg(ORG.getOrgId());//获取当前上传用户的组织id
        ...
        this.save(safety);//mybatisplus的保存到数据库中的用法
