...
public class DevFileServiceImpl extends ServiceImpl<DevFileMapper, DevFile> implements DevFileService {
    ...
    @Override
    public String uploadReturnId(String engine, MultipartFile file, String FileSource) {
        return this.storageFile(engine, file,FileSource, true);
    }

    @Override
    public String uploadReturnUrl(String engine, MultipartFile file, String FileSource) {
        return this.storageFile(engine, file, FileSource,false);
    }
    ...
    @Override
    public Page<DevFile> file(DevFilePageParam devFilePageParam,String FileSource) {//新增
        List<String> loginUserDataScope = StpLoginUserUtil.getLoginUserDataScope();
        SaBaseLoginUser user = StpLoginUserUtil.getLoginUser();
        QueryWrapper<DevFile> queryWrapper = new QueryWrapper<>();
        if (!user.getAccount().equals("superAdmin")) {
            if (ObjectUtil.isNotEmpty(loginUserDataScope)) {
                queryWrapper.lambda().in(DevFile::getOrgId, loginUserDataScope).eq(DevFile::getFileSource,FileSource);
            } else {
                queryWrapper.lambda().eq(DevFile::getOrgId, StpUtil.getLoginIdAsString()).eq(DevFile::getFileSource,FileSource);
            }
        }
        if (ObjectUtil.isNotEmpty(devFilePageParam.getOrgId())) {
            BizOrg org = bizOrgService.queryEntity(devFilePageParam.getOrgId());
            if (!org.getParentId().equals("0"))
                queryWrapper.lambda().eq(DevFile::getOrgId, devFilePageParam.getOrgId()).eq(DevFile::getFileSource,FileSource);
        }

        return this.page(CommonPageRequest.defaultPage(),  queryWrapper.lambda().eq(DevFile::getFileSource,FileSource));//根据FileSoure选择4种
    }
    /**
     * 存储文件
     *
     ...
     **/
    private String storageFile(String engine, MultipartFile file,String FileSource,boolean returnFileId) {
        ...
        if(engine.equals(DevFileEngineTypeEnum.LOCAL.getValue())) {
            ...
            storageUrl = DevFileLocalUtil.storageFileWithReturnUrl(bucketName, genFileKey(fileId, file), file);//返回图片文件绝对路径
        ...
        devFile.setFileSource(FileSource);//文件源
        // 设置文件id
    ...
    @Override
    public byte[] getImage(DevFileIdParam devFileIdParam) throws  Exception  {

        DevFile devFile = this.queryEntity(devFileIdParam.getId());
        File file = new File(devFile.getStoragePath());
    ...
    public byte[] getImage(String id) throws  Exception  {
        ...
        DevFile devFile = this.queryEntity(id);
        File file = new File(devFile.getStoragePath());
        FileInputStream inputStream = new FileInputStream(file);
        byte[] bytes = new byte[inputStream.available()];
        inputStream.read(bytes, 0, inputStream.available());
